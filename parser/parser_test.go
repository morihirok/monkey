...
import (
	...
	"github.com/morihirok/monkey/ast"
...
func TestBooleanExpression(t *testing.T) {
	...
	if bl.Value != true {
		t.Errorf("bl.Value not %t. got=%t", true, bl.Value)
	}
	if bl.TokenLiteral() != "true" {
		t.Errorf("bl.TokenLiteral not %t. got=%s", true, bl.TokenLiteral())
	}
}
...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
	if !ok {
		t.Fatalf("program.Statements[0] is not ast.ExpressionsStatement. got=%T", program.Statements[0])
	}
